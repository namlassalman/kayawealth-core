...
def _render_client_tools(st, backend_url: str) -> None:
    ...
    category_filter = st.selectbox("Filter by Category:", ["", "tax_planning", "risk_management", "portfolio_rebalancing", "macro_economics"], key="hybrid_cat")
    ...
    if search_query:
        ...
        try:
            ...
            st.caption(f"📊 Pools: Keyword [{pools['keyword_pool_size']}] | Vector [{pools['semantic_pool_size']}]")
            for index, match in enumerate(data["results"][:3]):
                with st.expander(f"Match {index + 1}: {match['id']}"):
    ...
    if st.checkbox("Show 1,000 document clusters", key="show_cluster_map"):
        try:
